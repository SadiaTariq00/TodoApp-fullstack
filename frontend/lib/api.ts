...
class ApiClient {
  ...
  async deleteTask(id: number): Promise<ApiResponse<{ message: string }>> {
    ...
    try {
      await this.client.delete(`/api/${userId}/tasks/${id}`);
      return { success: true, data: { message: "Task deleted successfully" }, status: 200 };
    } catch (err: any) {
      return this.handleError(err, "Failed to delete task");
    }
  }
  ...
  async toggleTaskCompletion(id: number): Promise<ApiResponse<Task>> {
    ...
    if (!userId) return { success: false, error: "Not authenticated", status: 401 };
    try {
      const res = await this.client.patch(`/api/${userId}/tasks/${id}/complete`);
      return { success: true, data: res.data, status: 200 };
    } catch (err: any) {
      return this.handleError(err, "Failed to toggle task completion");
    }
  }
